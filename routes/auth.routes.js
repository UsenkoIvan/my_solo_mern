  ...
  async (req, res) => {
    try {
      ...
      if (candidate) {
        return res
          .status(400)
          .json({ message: `User with email ${email} is exist` });
      }
      ...
      res
        .status(201)
        .json({ message: `New user with email ${email} is create` });
    } catch (err) {
      res
        .status(500)
        .json({ message: "Something wrong in DB, try again later" });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Something wrong in DB, try again late" });
    }
  }
);
